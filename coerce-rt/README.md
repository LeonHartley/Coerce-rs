# Coerce-rs [![crates.io](http://meritbadge.herokuapp.com/coerce_rt)](https://crates.io/crates/coerce_rt) ![coerce-rs](https://action-badges.now.sh/leonhartley/coerce-rs)
...
Actors can be stopped and actor references can be retrieved by ID (currently `Uuid`) from anywhere in your application. Anonymous actors are automatically dropped (and `Stopped`)
...
pub async fn run() {
    ...
    let timer = Timer::start(actor_ref.clone(), Duration::from_secs(5), TimerTick(hello_world));
