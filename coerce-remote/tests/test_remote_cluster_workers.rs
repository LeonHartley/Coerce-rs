...
use coerce_remote::cluster::workers::{ClusterWorker, ClusterWorkers, WorkerStore, WorkerStoreErr, GetActiveWorkers};
...
pub async fn test_remote_worker_store() {
    ...
    let mut workers = context
        .new_anon_actor(ClusterWorkers::new(TestWorkerStore))
        .await
        .unwrap();
